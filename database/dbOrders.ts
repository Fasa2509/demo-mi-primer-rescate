...
export const createNewOrder = async ( userId: string, cart: ICartProduct[], total: number, shippingAddress: IAddress, contact: IContact ): Promise<{ error: boolean; message: string; }> => {
    ...
    if ( !userId || cart.length < 1 || total === 0 || Object.entries( shippingAddress ).filter(d => d[1]).length < 2 || Object.values( contact ).filter(c => c).length < 1 ) return { error: true, message: 'Falta información de la orden' };
